...
bool Image<T>::read_nii() {
    ...
    switch (inputDataType) {
        ...
        case UINT8_DT:         reIdx ? convert<T,uint8_t>    (data,nim->data,imgDims,indexOrder,dataScaler,dataOffset) : convert<T,uint8_t>    (data,nim->data,imgDims,dataScaler,dataOffset);    break;
        case INT8_DT:          reIdx ? convert<T,int8_t>     (data,nim->data,imgDims,indexOrder,dataScaler,dataOffset) : convert<T,int8_t>     (data,nim->data,imgDims,dataScaler,dataOffset);    break;
        case UINT16_DT:        reIdx ? convert<T,uint16_t>   (data,nim->data,imgDims,indexOrder,dataScaler,dataOffset) : convert<T,uint16_t>   (data,nim->data,imgDims,dataScaler,dataOffset);    break;
        case INT16_DT:         reIdx ? convert<T,int16_t>    (data,nim->data,imgDims,indexOrder,dataScaler,dataOffset) : convert<T,int16_t>    (data,nim->data,imgDims,dataScaler,dataOffset);    break;
        case UINT32_DT:        reIdx ? convert<T,uint32_t>   (data,nim->data,imgDims,indexOrder,dataScaler,dataOffset) : convert<T,uint32_t>   (data,nim->data,imgDims,dataScaler,dataOffset);    break;
        case INT32_DT:         reIdx ? convert<T,int32_t>    (data,nim->data,imgDims,indexOrder,dataScaler,dataOffset) : convert<T,int32_t>    (data,nim->data,imgDims,dataScaler,dataOffset);    break;
        case UINT64_DT:        reIdx ? convert<T,uint64_t>   (data,nim->data,imgDims,indexOrder,dataScaler,dataOffset) : convert<T,uint64_t>   (data,nim->data,imgDims,dataScaler,dataOffset);    break;
        case INT64_DT:         reIdx ? convert<T,int64_t>    (data,nim->data,imgDims,indexOrder,dataScaler,dataOffset) : convert<T,int64_t>    (data,nim->data,imgDims,dataScaler,dataOffset);    break;
        case FLOAT32_DT:       reIdx ? convert<T,float>      (data,nim->data,imgDims,indexOrder,dataScaler,dataOffset) : convert<T,float>      (data,nim->data,imgDims,dataScaler,dataOffset);    break;
        case FLOAT64_DT:       reIdx ? convert<T,double>     (data,nim->data,imgDims,indexOrder,dataScaler,dataOffset) : convert<T,double>     (data,nim->data,imgDims,dataScaler,dataOffset);    break;
        case FLOAT128_DT:      reIdx ? convert<T,long double>(data,nim->data,imgDims,indexOrder,dataScaler,dataOffset) : convert<T,long double>(data,nim->data,imgDims,dataScaler,dataOffset);    break;
